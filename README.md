...
- **حساب/ترحيل نقاط الولاء تلقائيًا**: تم ربطها تلقائياً الآن حيث تُضاف النقاط لحساب العميل فور الدفع الكامل للفاتورة.
- **تصدير تقارير حقيقي (PDF/Excel)**: تم ربط توليد الملفات وتخزينها على S3 مع إتاحتها للتنزيل بروابط مؤقتة أو دائمة عبر الـ URL.
- **رفع/تنزيل ملف النسخة الاحتياطية الفعلي**: تم تطبيقها الآن حيث تُنفذ `mysqldump` وتُخزّن الناتج على S3 عبر العامل الخلفي. في حال عدم توفر أداة `mysqldump` على السيرفر، يتم إنشاء ملف JSON بديل.
