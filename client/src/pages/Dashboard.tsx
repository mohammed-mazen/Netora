...
export default function Dashboard() {
  ...
  return <div className="netora-grid min-h-screen bg-[#f5f7fb] text-slate-900">
    ...
    {searchOpen && <div className="fixed inset-0 z-[60] grid place-items-start bg-slate-950/25 px-4 pt-24 backdrop-blur-sm" onClick={() => setSearchOpen(false)}><div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl" onClick={event => event.stopPropagation()}><div className="flex items-center gap-3 border-b border-slate-100 px-2 pb-3"><Command className="h-4 w-4 text-violet-600" /><input autoFocus placeholder="ابحث عن وحدة أو إجراء…" className="min-w-0 flex-1 border-0 bg-transparent text-sm outline-none placeholder:text-slate-400" /><button onClick={() => setSearchOpen(false)} className="rounded-md bg-slate-100 px-2 py-1 text-[10px] text-slate-500">Esc</button></div><div className="p-2"><p className="px-2 py-2 text-[10px] font-bold text-slate-400">إجراءات سريعة</p>
      <button onClick={() => { setSearchOpen(false); setActive("routers"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />إضافة راوتر</button>
      <button onClick={() => { setSearchOpen(false); setActive("cards"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />إنشاء دفعة بطاقات</button>
      <button onClick={() => { setSearchOpen(false); setActive("customers"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />إضافة عميل</button>
      <button onClick={() => { setSearchOpen(false); setActive("support"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />فتح تذكرة</button>
    </div></div></div>}
  </div>;
}
