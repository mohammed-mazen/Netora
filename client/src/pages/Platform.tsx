import {
lazy, Suspense, useEffect, useMemo, useState } from "react";
import {
toast } from "sonner";
import {
...
trpc } from "@/lib/trpc";
import {
TenantPlanUsageCard } from "@/components/TenantPlanUsageCard";
import {
...
const PlatformInvoicesPanel = lazy(() => import("@/components/PlatformBillingPanels").then(module => ({ default: module.PlatformInvoicesPanel })));
const PlatformPaymentsPanel = lazy(() => import("@/components/PlatformBillingPanels").then(module => ({ default: module.PlatformPaymentsPanel })));
...
const platformNavigation: { title: string; items: { key: ModuleKey; label: string; icon: LucideIcon; badge?: string }[] }[] = [
  ...
  { title: "الإدارة التشغيلية", items: [{ key: "platform_organizations", label: "مؤسسات المنصة", icon: Boxes }, { key: "platform_plans", label: "خطط الاشتراك", icon: BadgeDollarSign }, { key: "platform_subscriptions", label: "الاشتراكات النشطة", icon: CheckCircle2 }, { key: "platform_support", label: "تذاكر المنصة", icon: CircleAlert, badge: "جديد" }] }, { title: "المالية والفوترة", items: [{ key: "platform_invoices", label: "فواتير المنصة", icon: ReceiptText }, { key: "platform_payments", label: "سجل الدفعات", icon: CircleDollarSign }] }
];
...
const moduleDetails: Record<ModuleKey, { title: string; icon: LucideIcon; desc: string }> = {
  ...
  platform_invoices: { title: "فواتير المنصة", icon: ReceiptText, desc: "إدارة فواتير اشتراكات المؤسسات." },
  platform_payments: { title: "سجل الدفعات", icon: CircleDollarSign, desc: "سجل مدفوعات المؤسسات للمنصة." },
};
...
function Overview({ workspace, preview, tenant, isLoading, error }: { workspace: Workspace; preview: any; tenant: any; isLoading: boolean; error: boolean }) {
  ...
  const safeLocaleString = (val: any) => (val ?? 0).toLocaleString();
  return <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><StatCard title="المؤسسات المسجلة" value={safeLocaleString(stats.totalOrganizations)} icon={Boxes} color="text-violet-600" bg="bg-violet-50" border="border-violet-100" /><StatCard title="الاشتراكات النشطة" value={safeLocaleString(stats.activeOrganizations)} icon={CheckCircle2} color="text-emerald-600" bg="bg-emerald-50" border="border-emerald-100" /><StatCard title="العوائد الشهرية" value={`$${safeLocaleString(stats.totalPlatformRevenue)}`} icon={BadgeDollarSign} color="text-blue-600" bg="bg-blue-50" border="border-blue-100" /><StatCard title="تذاكر المنصة" value={safeLocaleString(stats.activePlatformTickets)} icon={CircleAlert} color="text-amber-600" bg="bg-amber-50" border="border-amber-100" /></div><div className="rounded-2xl border border-slate-100 bg-white p-5"><h3 className="text-sm font-bold text-slate-900">سجل عمليات المنصة (الحديثة)</h3><div className="mt-4 flex h-64 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50"><div className="text-center"><ShieldCheck className="mx-auto mb-3 h-8 w-8 text-slate-300" /><p className="text-sm font-semibold text-slate-500">واجهة المراقبة المركزية</p><p className="mt-1 text-[11px] text-slate-400">تُظهر نمو وإحصائيات المؤسسات على مستوى الخادم.</p></div></div></div></div>;
}
...
export default function Platform() {
  ...
  return <div className="netora-grid min-h-screen bg-[#f5f7fb] text-slate-900">
    ...
    <main className="min-h-screen lg:mr-[276px]"><header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-slate-200/70 bg-[#f5f7fb]/90 px-4 backdrop-blur-xl sm:px-6"><div className="flex items-center gap-3"><button aria-label="فتح القائمة" onClick={() => setSidebarOpen(true)} className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 lg:hidden"><PanelRight className="h-4 w-4" /></button><div><p className="text-[11px] font-semibold text-slate-400">الإدارة المركزية</p><h2 className="mt-0.5 text-sm font-bold text-slate-800">{activeTitle}</h2></div></div><div className="flex items-center gap-2"><button onClick={() => setSearchOpen(true)} className="hidden h-9 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 text-xs text-slate-400 hover:border-violet-200 sm:flex"><Search className="h-4 w-4" /><span>بحث سريع</span><kbd className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] text-slate-500">⌘ K</kbd></button><button aria-label="الإشعارات" className="relative grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"><Bell className="h-4 w-4" /></button><div className="hidden items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 sm:flex"><Gauge className="h-4 w-4 text-emerald-600" />الأنظمة متصلة</div></div></header>
      <div className="mx-auto max-w-[1560px] px-4 py-6 sm:px-6 lg:px-8">{active === "overview" ? <div className="space-y-5"><TenantContextCard isAuthenticated={isAuthenticated} isLoading={membershipsQuery.isLoading} error={Boolean(membershipsQuery.error)} memberships={membershipsQuery.data || []} selectedSlug={selectedOrganizationSlug} onSelect={setSelectedOrganizationSlug} /><Overview workspace={workspace} preview={previewQuery.data} tenant={tenantOverviewQuery.data} isLoading={authLoading || previewQuery.isLoading || tenantOverviewQuery.isLoading} error={(Boolean(previewQuery.error) || Boolean(tenantOverviewQuery.error)) && isAuthenticated} /></div> : <ModuleWorkspace active={active} workspace={workspace} organizationSlug={selectedOrganizationSlug} isPlatformAdmin={user?.role === "admin"} />}</div>
    </main>
    {searchOpen && <div className="fixed inset-0 z-[60] grid place-items-start bg-slate-950/25 px-4 pt-24 backdrop-blur-sm" onClick={() => setSearchOpen(false)}><div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl" onClick={event => event.stopPropagation()}><div className="flex items-center gap-3 border-b border-slate-100 px-2 pb-3"><Command className="h-4 w-4 text-violet-600" /><input autoFocus placeholder="ابحث عن وحدة أو إجراء…" className="min-w-0 flex-1 border-0 bg-transparent text-sm outline-none placeholder:text-slate-400" /><button onClick={() => setSearchOpen(false)} className="rounded-md bg-slate-100 px-2 py-1 text-[10px] text-slate-500">Esc</button></div><div className="p-2"><p className="px-2 py-2 text-[10px] font-bold text-slate-400">إجراءات سريعة</p>
      <button onClick={() => { setSearchOpen(false); setActive("platform_organizations"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />إدارة المؤسسات</button>
      <button onClick={() => { setSearchOpen(false); setActive("platform_plans"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />خطط المنصة</button>
      <button onClick={() => { setSearchOpen(false); setActive("platform_support"); }} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-right text-xs font-semibold text-slate-700 hover:bg-violet-50 hover:text-violet-700"><Plus className="h-4 w-4" />تذاكر الدعم الفني</button>
    </div></div></div>}
  </div>;
}
